...

{
	...
	*/
	...
	//
	SetupComm(m_hComm,1024*256,1024*256);
	// flush the port
	///终止读写并清空接收和发送
...
{
#ifndef IS_DOMESTIC
	OutputDebugString(">>>foreign_1\n");
	CString neg = "";
	for (int i = 0; i<n; i++)
	{
		CString strAscii;
		strAscii.Format("%d",string[i]);
		int num = strtol(strAscii,NULL,10);
		neg += (unsigned char)(~num);
		//CString strHex;
		//strHex.Format("%02x",(unsigned char)(~num));
	}
	string = neg.GetBuffer(neg.GetLength());
#else
	OutputDebugString(">>>domestic_1\n");
#endif
